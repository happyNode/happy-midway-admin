...
import { CreateUserDto, UpdatePasswordDto, UpdatePersonInfoDto, UpdateUserDto } from "../model/dto/user";
...
export class UserService extends BaseService {
  ...
  async getAccountInfo(uid: number): Promise<IAccountInfo | null> {
    const user: UserEntity | undefined = await this.mapping.findOne({ id: uid });
    if (!isEmpty(user)) {
      return {
        name: user.name,
  ...
  async info(
    id: number
  ): Promise<(UserEntity & { roles: number[] }) | never> {
    const user: any = await this.mapping.findOne(id);
    if (isEmpty(user)) {
      throw new Error('unfind this user info');
    }
    const roleRows = await this.userRoleMapping.findAll({ userId: user!.userId });
